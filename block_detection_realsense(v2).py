...
class BlockDetector:
    ...
    def detect(self, frame, depth_image, camera) -> List[Block]:
        ...
        # 밝은 모드만: threshold 이상이 흰색 (THRESH_BINARY)
    ...
    for i, block in enumerate(g_blocks):
        ...
        if bx <= x <= bx+bw and by <= y <= by+bh:
            ...
            if point_3d and point_3d[2] > 0:
                ...
                print(f"  📏 수직 거리: {depth*1000:.1f} mm ({depth*100:.1f} cm)")
                
                bx, by, bw, bh = block.bbox
                real_w, real_h = g_camera.calc_real_size(bw, bh, depth)
                ...
                print(f"\n  🤖 그리퍼 참고:")
                print(f"     - 그리퍼 벌림: ~{side_length + 5:.0f} mm")
                print(f"     - 하강 거리: ~{depth*1000:.0f} mm")
            else:
                print("\n  ⚠️ 뎁스 정보 없음")
